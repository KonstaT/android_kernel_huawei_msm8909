...
#include <linux/fb.h>
//add by pangle for tp charge interface updatedate 20150325  
                            ...
                            char *buf)
{
       ...
       __stringify(EV_KEY) ":" __stringify(KEY_BACK)  ":80:900:170:40"
       ":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":240:900:170:40"
       ":" __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":400:900:170:40"
       "\n");
}
...
{	
		switch(gesture_id)
               	...
               	case 0x24:
					...
					input_report_key(data->input_dev, KEY_ENTER, 1);  //modify by pangle at 20150408
                                   input_sync(data->input_dev);
                                   input_report_key(data->input_dev, KEY_ENTER, 0);  //modify by pangle at 20150408
                                   input_sync(data->input_dev);		   
				break;
	...
	for (i = 0; i < event->touch_point; i++) {
		...
		if (event->pressure == FT_PRESS) {
			...
			input_report_abs(data->input_dev, ABS_MT_POSITION_Y,
					event->au16_y[i]);
			//comment by pangle at 20150812
			//input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR,
			//		event->pressure);
		}
	    //printk(KERN_INFO "----------------X=%d.Y=%d\n",event->au16_x[i],event->au16_y[i]);
		//add by pangle for back&menu key function 20150508 begin
		if(event->au16_y[i] > 900)
		{
			input_report_key(data->input_dev, KEY_F10, 1);  //modify by pangle at 20150408
            input_sync(data->input_dev);
            input_report_key(data->input_dev, KEY_F10, 0);  //modify by pangle at 20150408
            input_sync(data->input_dev);	
		}
		//add by pangle for back&menu key function 20150508 end
	}
...
//modify by pangle for charger interference to tp 20150325	begin	
#ifdef FT_ENABLE_GESTURE_WAKEUP
	 if(!gesture_flag)
#endif
	 {
		...
		{
			//printk("pangle trace charge insert\n");
...
//modify by pangle for charger interference to tp 20150325	end
...
#endif
//add by shihuijun for resume and suspend after LCD 20150401 start
...
//shihuijun  added end for suspend and resume after LCD 20150401 end
...
//modify by chenchen for tp crash (SW00074483) 20140827 begin
	...
	//printk("pangle trace loading_fw:%d",ft5x36_ts->loading_fw);
...
//added by pangle for gesture 20141027 begin
#ifdef FT_ENABLE_GESTURE_WAKEUP
   if(!gesture_flag)
#endif
   {	
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		blank = evdata->data;
		#if 0//yuquan modified:add work for suspend and resume.
		if (*blank == FB_BLANK_UNBLANK)
			ft5x36_ts_resume(&ft5x36_data->client->dev);
		else if (*blank == FB_BLANK_POWERDOWN)
			ft5x36_ts_suspend(&ft5x36_data->client->dev);
		#else
		ft5x36_data->pm_status=*blank;
	      spin_lock_irqsave(&ft5x36_data->lock, flags);
	      queue_delayed_work(ft5x36_data->fts_pm_wq,
				&ft5x36_data->fts_pm_work,msecs_to_jiffies(120));
	    // schedule_delayed_work(&ft5x36_data->fts_pm_work, msecs_to_jiffies(120));	// restart timer
	     spin_unlock_irqrestore(&ft5x36_data->lock, flags);
		#endif
	}
...
//added by pangle for gesture 20140917
	...
	input_mt_init_slots(input_dev,pdata->max_touch_point,0);
	   ...
	   input_set_capability(input_dev, EV_KEY, KEY_F10);  //add by pangle for back&menu key function 20150508
//added by pangle for gesture 20140917 begin
#ifdef FT_ENABLE_GESTURE_WAKEUP
       input_set_capability(input_dev, EV_KEY, KEY_ENTER);   //modify by pangle at 20150408
