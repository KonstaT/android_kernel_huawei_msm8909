...
#include "ft5x36_ts.h"
#include <linux/gpio.h>  //add by pangle for fw upgrade at 20150501
...
int fts_read_project_code(struct i2c_client * client, char * pProjectCode) {
	...
	u8 hw_rest_loop=0;  //add by pangle for fw upgrade at 20150501
    u8 hw_flag=0;
	//int      i_ret;
...
//modify by pangle for fw upgrade at 20150501 begin
    hw_rest_loop = FTS_UPGRADE_LOOP;
	for (i = 0; i < hw_rest_loop; i++) {
		msleep(100);
	...
	if (i >= hw_rest_loop)
		return -EIO;
//modify by pangle for fw upgrade at 20150501 end
	auc_i2c_write_buf[0] = 0xcd;
	fts_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 1);
